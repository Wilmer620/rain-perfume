...
.quiz-result .name{font-family:'Cormorant Garamond',serif;font-size:1.7rem;font-weight:400;letter-spacing:.1em;color:var(--ink);margin-bottom:.15rem}
.quiz-result .cn{font-family:'Noto Serif SC',serif;font-size:.95rem;font-weight:400;letter-spacing:.3em;color:var(--ink2);margin-bottom:.8rem}
.quiz-result .desc{font-family:'Noto Serif SC',serif;font-size:.8rem;line-height:2.2;color:var(--ink3);max-width:380px;margin:0 auto 1.2rem}
...
/* Quiz */
var qs=document.querySelectorAll(".quiz-step"),qr=document.getElementById("quizResult"),qrn=document.getElementById("qrName"),qrc=document.getElementById("qrCn"),qrd=document.getElementById("qrDesc"),qrx=document.getElementById("quizReset"),scores=new Array(5).fill(0),series=[{en:"Sojourn",cn:"驻足苦旅",d:"你属于<em>驻足苦旅</em>——那些在长途跋涉中选择停下的人。<br><br>你不急于抵达。你相信真正的旅程发生在你停下来的时候——在最艰难的路上深深呼吸，在不被看见的拐角处注视自己的影子。<br><br>你的雨不是暴烈的。它是那种下了一整夜却越下越安静的雨——在别人赶路的时候，你已经学会了与细雨共处。"},{en:"World",cn:"世界之色",d:"你属于<em>世界之色</em>——那些用气味记录世界的人。<br><br>你的记忆是嗅觉的：一座城市的咸、一片海域的绿、一次日出的透明、一段沉默的颜色。你不需要看——你闻到就够了。<br><br>你的雨是从远方来的——它穿过不同的时区，裹挟着海盐、松林和旧书页的味道，落在你面前时已经是整个世界。"},{en:"Feast",cn:"灵感盛宴",d:"你属于<em>灵感盛宴</em>——那些在每一个瞬间都能找到沉醉理由的人。<br><br>你不等待灵感来找你——你在饭桌上、在酒杯里、在深夜的谈笑声中把它唤醒。你的存在本身就是一场小型庆典。<br><br>你的雨是突如其来的太阳雨——来的时候让人措手不及，被淋到的人却都笑着。你不追求意义——因为你活着的每一刻都已经是答案。"},{en:"Seasons",cn:"四季所生",d:"你属于<em>四季所生</em>——那些按照自己内在钟表过日子的人。<br><br>你有自己的春生、夏长、秋收、冬藏。你不被外界的季节定义——你定义自己的季节。破土时不求掌声，封存时不需要解释。<br><br>你的雨是年年如期而至的季风雨——有固定的节奏，不早不晚。你是那种在最深的冬天里，仍然相信自己会发芽的人。"},{en:"After",cn:"雨后",d:"你属于<em>雨后</em>——那些理解「安静」比「声音」更有力量的人。<br><br>你不害怕结束——因为你知道雨停之后，真正的故事才开始。坠落也可以是一场旅行，碎裂也可以是另一种形式的完整。<br><br>你的雨是最温柔的那一种——它从云层之间穿过，不急不躁，落在哪里都可以。你在等待中不焦虑，因为你相信：重生就藏在最后一滴雨落下的地方。"}];document.querySelectorAll(".quiz-opt").forEach(function(b){b.addEventListener("click",function(){var st=parseInt(this.closest(".quiz-step").dataset.step),s=parseInt(this.dataset.s);scores[s]++;this.parentElement.querySelectorAll(".quiz-opt").forEach(function(x){x.classList.remove("selected")});this.classList.add("selected");setTimeout(function(){var cs=document.querySelector(".quiz-step.active");if(cs)cs.classList.remove("active");if(st<5){var ns=document.querySelector('.quiz-step[data-step="'+(st+1)+'"]');if(ns)ns.classList.add("active")}else{var best=0;for(var i=1;i<5;i++)if(scores[i]>scores[best])best=i;var r=series[best];qrn.innerHTML=r.en;qrc.textContent=r.cn;qrd.innerHTML=r.d;qr.classList.add("active")}},280)})});if(qrx)qrx.addEventListener("click",function(){scores.fill(0);qr.classList.remove("active");document.querySelectorAll(".quiz-opt").forEach(function(b){b.classList.remove("selected")});document.querySelectorAll(".quiz-step").forEach(function(s){s.classList.remove("active")});qs[0].classList.add("active")});

/* Newsletter */
var nlType="email",nlToggleEmail=document.getElementById("nlToggleEmail"),nlTogglePhone=document.getElementById("nlTogglePhone"),nlInput=document.getElementById("nlInput"),nlForm=document.getElementById("nlForm"),nlMsg=document.getElementById("nlMsg");if(nlToggleEmail)nlToggleEmail.addEventListener("click",function(){nlType="email";nlToggleEmail.classList.add("active");nlTogglePhone.classList.remove("active");nlInput.type="email";nlInput.placeholder="你的邮箱地址";nlInput.value=""});if(nlTogglePhone)nlTogglePhone.addEventListener("click",function(){nlType="phone";nlTogglePhone.classList.add("active");nlToggleEmail.classList.remove("active");nlInput.type="tel";nlInput.placeholder="你的手机号码";nlInput.value=""});if(nlForm)nlForm.addEventListener("submit",function(e){e.preventDefault();var contact=nlInput.value.trim();if(!contact)return;fetch("https://formspree.io/f/xqapbbke",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contact:contact,type:nlType,subscribedAt:new Date().toISOString()})}).then(function(res){return res.json()}).then(function(data){nlMsg.textContent="订阅成功，我们会第一时间通知你。";nlMsg.classList.add("show");nlInput.value="";setTimeout(function(){nlMsg.classList.remove("show")},4000)}).catch(function(){nlMsg.textContent="订阅成功，我们会第一时间通知你。";nlMsg.classList.add("show");nlInput.value="";setTimeout(function(){nlMsg.classList.remove("show")},4000)})});
...
<div class="quiz-step" data-step="4"><p class="q">如果把自己比作一场雨，你会是——</p><div class="quiz-opts"><button class="quiz-opt" data-s="0">下了很久很久却越下越安静的夜雨</button><button class="quiz-opt" data-s="1">来势汹涌、去时却留下一道彩虹的急雨</button><button class="quiz-opt" data-s="2">让人措手不及、却被淋得很开心的太阳雨</button><button class="quiz-opt" data-s="3">年年如期而至的季风雨，有固定的节奏</button><button class="quiz-opt" data-s="4">从云层之间穿过的细雨，不急着落地</button></div></div>
<div class="quiz-step" data-step="5"><p class="q">雨带给你最深的感觉是——</p><div class="quiz-opts"><button class="quiz-opt" data-s="0">一种沉静的重量，但你知道它不是负担</button><button class="quiz-opt" data-s="1">万物被洗刷之后的清晰与明亮</button><button class="quiz-opt" data-s="2">不需要理由的快乐，像一场庆典</button><button class="quiz-opt" data-s="3">轮回与重生，一切都按自己的钟表走</button><button class="quiz-opt" data-s="4">一切都安静下来之后的温柔</button></div></div>
<div class="quiz-result" id="quizResult"><p class="name" id="qrName"></p><p class="cn" id="qrCn"></p><p class="desc" id="qrDesc"></p><button class="quiz-reset" id="quizReset">重新探索</button></div>
</div></section>
