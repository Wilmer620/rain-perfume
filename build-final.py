...
function pickSet(){var pool=[];for(var i=0;i<allSets.length;i++){if(used.indexOf(i)<0)pool.push(i)}if(pool.length===0){used=[];for(var j=0;j<allSets.length;j++)pool.push(j)}var pick=pool[Math.floor(Math.random()*pool.length)];used.push(pick);return pick}
